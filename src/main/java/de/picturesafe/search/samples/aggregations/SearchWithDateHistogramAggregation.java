...
@Component
@ComponentScan
public class SearchWithDateHistogramAggregation implements PicturesafeSearchSample {

    private static final Logger LOGGER = LoggerFactory.getLogger(SearchWithDateHistogramAggregation.class);

    @Autowired
    private SingleIndexElasticsearchService singleIndexElasticsearchService;

    public static void main(String[] args) {
        try (AnnotationConfigApplicationContext ctx = new AnnotationConfigApplicationContext(SearchWithDateHistogramAggregation.class)) {
            final SearchWithDateHistogramAggregation facet = ctx.getBean(SearchWithDateHistogramAggregation.class);
            facet.run();
        }
    }
    ...
    private void createTestRecords() {
        ...
        // Insert 5 test records with create date 'one year ago'
        singleIndexElasticsearchService.addToIndex(DataChangeProcessingMode.BLOCKING, LongStream.range(31, 36).boxed()
                .map(id -> DocumentBuilder.id(id).put("title", "This is a test title " + id).put("created", Date.from(oneYearAgo)).build())
                .collect(Collectors.toList()));
    }
    ...
    private SearchParameter createSearchParameter() {
        return SearchParameter.builder().pageSize(10).pageIndex(1)
                ...
                .aggregations(DateHistogramAggregation.field("created").interval(CALENDAR, "1y").format("yyyy").name("years"))
                .build();
    }
